---
title: "Light Romanian Ciorbă de Perișoare (Lean Turkey Meatball Soup)"
description: "This lightened-up version of traditional Romanian meatball soup swaps fatty pork for lean turkey, keeping the vibrant sour tang intact. Packed with root vegetables and fresh herbs, it offers plenty of satisfying protein without feeling heavy."
image: "/images/light-romanian-ciorba-de-perisoare-meatball-soup.webp"
course: lunch
cuisine: "Romanian"
prepTime: 15
cookTime: 30
totalTime: 45
servings: 4
calories: 280
protein: 18
carbs: 24
fat: 8
fibre: 5
tags: ["romanian", "soup", "high-protein", "under-300-kcal"]
category: "healthy-soups"
tip: "Roll the meatballs with damp hands to stop the lean turkey mixture from sticking to your fingers."
publishDate: 2026-09-21
featured: false
ingredients:
  - amount: "400g"
    name: "lean turkey mince"
  - amount: "1 medium"
    name: "onion (half grated, half chopped)"
  - amount: "1 large"
    name: "carrot, diced"
  - amount: "1 stick"
    name: "celery, diced"
  - amount: "1 small"
    name: "red bell pepper, diced"
  - amount: "1 tbsp"
    name: "olive oil"
  - amount: "4 tbsp"
    name: "uncooked white rice, rinsed"
  - amount: "1 large"
    name: "egg white"
  - amount: "1.2 litres"
    name: "chicken broth (low sodium)"
  - amount: "250ml"
    name: "passata or crushed tomatoes"
  - amount: "3 tbsp"
    name: "fresh lemon juice"
  - amount: "1 large handful"
    name: "fresh lovage or celery leaves, chopped"
  - amount: "1 large handful"
    name: "fresh parsley, chopped"
steps:
  - "Place the turkey mince, grated half of the onion, rinsed rice, and egg white into a mixing bowl."
  - "Season the meat mixture with a pinch of salt and black pepper, mixing gently with your hands until just combined, then roll into bite-sized meatballs using wet hands."
  - "Heat the olive oil in a large pot over medium heat, then sauté the remaining chopped onion, carrot, celery, and bell pepper until they soften and smell sweet, about 5 minutes."
  - "Pour in the chicken broth and passata, bringing the liquid to a gentle simmer."
  - "Carefully drop the turkey meatballs into the simmering broth one by one, reducing the heat to medium-low."
  - "Cover the pot and let the soup simmer for 20 minutes, or until the meatballs are cooked through and the rice inside them is tender."
  - "Stir in the lemon juice, fresh lovage, and parsley, simmering for 2 more minutes before removing from the heat."
---

Growing up, a steaming bowl of ciorbă de perișoare was a staple on our family table. Its distinctive sour broth always cut through the rich, heavy pork meatballs my grandmother insisted on making. I can still smell the sharp, citrusy tang of the boiling liquid mingling with earthy root vegetables on a Sunday afternoon. While I still adore those traditional flavours, my stomach prefers a lighter touch these days. 

To keep that nostalgic, comforting essence without the post-meal sluggishness, I swap the traditional fatty pork for lean turkey mince. It binds beautifully with a little egg white and rice, turning into delicate, tender little spheres that poach gently in the tomato-tinted broth. The real secret of Romanian ciorbă is the souring agent. Authentic recipes call for a fermented wheat bran liquid called borș, but I've found that a generous squeeze of fresh lemon juice creates that exact lip-smacking acidity I crave.

This soup fits perfectly into a balanced routine because it relies on the natural sweetness of carrots and bell peppers, lean protein from the turkey, and a bit of filling starch from the rice hidden inside the meatballs. You get a complete, satisfying meal in one bowl that won't weigh you down.

## How I Serve It

I always serve this with a dollop of low-fat Greek yoghurt stirred right into the bowl, replacing the traditional heavy sour cream. It creates a creamy, velvety finish that beautifully mellows the sharp lemon broth. A slice of dense, crusty sourdough bread on the side is perfect for mopping up every last drop of the rust-coloured liquid. If you like a bit of heat, bite into a raw green chilli between spoonfuls of soup, which is exactly how they do it in Bucharest.

## Storage and Easy Swaps

The meatballs actually absorb more flavour as they sit, making this soup taste even better the next day. I store leftovers in a glass airtight container in the fridge for up to three days. When reheating, do it gently on the stove rather than boiling it rapidly, otherwise the meatballs might break apart.

If you can't find lovage, a leafy herb that tastes like intense celery and is crucial for authentic Romanian flavour, just use the inner, leafy tops of a bunch of celery mixed with plenty of fresh parsley. You can also experiment with the meat. Lean chicken mince works just as well as turkey if that's what you have sitting in your fridge right now.